...
class Home extends BaseController
{
    ...
    public function chadash(){
        ...
        $data['event'] = $user
        ->orderBy('ev_id', 'DESC')
        ->where('ev_organiser', session()->get('charity_id'))
        ->where('ev_status', 1)
        ->countAllResults();
        ...
        $data['rakib'] = $user->orderBy('don_id', 'DESC')->where('ddonetion.charity', session()->get('charity_id'))->first();

        return view('backend/chadash',$data);
    }

    public function eventdonation($id = null)
    {
        $user = new EventModel();
        $user->join('charity', 'event.ev_organiser = charity.charity_id');
        $user->join('totalamount', 'event.ev_id = totalamount.event_id', 'left');
